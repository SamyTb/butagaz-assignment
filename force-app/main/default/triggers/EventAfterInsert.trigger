trigger EventAfterInsert on Event (after insert) {
    if (Trigger.IsAfter && Trigger.IsInsert) {
        System.Debug('##>>> Event.AfterInsert : BEGIN <<< run by ' + UserInfo.getName());
        ...
        if (PAD.canTrigger('Event.incrementNbVisiteParAnPourPrescripteur')){
          //  AP_Event.incrementNbVisiteParAnPourPrescripteur(trigger.newMap);
        }
      ...
      /* if (PAD.canTrigger('Event.putSObjectInfoPTLGEBInEvent')){
        ...
        System.Debug('##>>> Event.AfterInsert : FALSE TRIGGER <<<');
    }
}